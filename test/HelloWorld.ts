...
describe("HelloWorld", function () {
  ...
  it("Should change text correctly", async function () {
    // TODO
    const {helloWorldContract} = await loadFixture(deployContract);
    const newText = "anything";
    const newTextBytes = ethers.encodeBytes32String(newText);
    const tx = await helloWorldContract.setText(newTextBytes);
    await tx.wait();
    const textFromContract = await helloWorldContract.helloWorld();
    expect (newTextBytes).to.eq(textFromContract);
    // throw Error("Not implemented");
  });
});